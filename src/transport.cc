...
void convection(struct Transport *trans) {
    ...
    register int t;
  int n_subst,sbi,elm_pos,rank,i;
  FILE *fw_chem; //clean the output file for chemistry
    ...
    double ***pconc;
    double ***conc;
  /*int chem_step = problem->chem_steps;
  PetscScalar **conc_mob_arr; //arrays for handling data inside of chemistry module
  PetscScalar **conc_immob_arr;
  PetscScalar **sorb_mob_arr;
  PetscScalar **sorb_immob_arr;*/

  n_subst = mesh->n_substances;

//======================================
//    CHEMISTRY DATA ALLOCATION
//======================================
  /*if(problem->semchemie_on == true)
  {
    conc_mob_arr=(PetscScalar**)xmalloc(n_subst * (sizeof (PetscScalar*)));
  
    if((trans->sub_problem & 1) == 1)
     conc_immob_arr=(PetscScalar**)xmalloc(n_subst * (sizeof (PetscScalar*)));
    else
     conc_immob_arr = NULL;

    if((trans->sub_problem & 2) == 2)
     sorb_mob_arr=(PetscScalar**)xmalloc(n_subst * (sizeof (PetscScalar*)));
    else
     sorb_mob_arr = NULL;

    if((trans->sub_problem & 3) == 3)
     sorb_immob_arr=(PetscScalar**)xmalloc(n_subst * (sizeof (PetscScalar*)));
    else
     sorb_immob_arr = NULL;
  }*/
//======================================
...
//======================================
	...
	if(problem->transport->sorption == true)
	{
	  for(sbi = 0;sbi < n_subst;sbi++) VecGetArray(trans->vconc_im_so[sbi],&sorb_immob_arr[sbi]);
       ...
       che_vypocetchemie(problem, pconc[MOBILE], pconc[IMMOBILE], pconc[MOBILE_SORB], pconc[IMMOBILE_SORB]);
...
//======================================
  /*if(problem->semchemie_on == true)
  {
    ...
    if((trans->sub_problem & 1) == 1)
    {
     for(i = 0; i < n_subst; i++) 
     {
       free(conc_immob_arr[i]);
       conc_immob_arr[i] = NULL;
     }
     free(conc_immob_arr);
     conc_immob_arr = NULL;
    }
    ...
    for(i = 0; i < n_subst; i++) 
    {
      free(sorb_immob_arr[i]);
      sorb_immob_arr[i] = NULL;
    }
     free(sorb_immob_arr);
     sorb_immob_arr = NULL;
    }
