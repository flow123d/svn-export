...
int Profiler::find_child(const CodePoint &cp) {
    ...
    do {
        ...
        ASSERT( child_idx < timers_.size(), "Profiler error.");
...
void Profiler::stop_timer(int timer_index) {
    ...
    ASSERT( timer_index < timers_.size(), "Profiler error." );
...
void Profiler::output(MPI_Comm comm) {
            ...
            string full_fname =  IONameHandler::get_instance()->get_output_file_name(string(filename));
