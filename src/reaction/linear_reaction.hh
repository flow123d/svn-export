...
public:
    /**
     *   \brief Default constructor just because of boost::serialization
     */
    Linear_reaction();
...
//        }
        ...
        ar & dual_porosity_on;
        ////            ar & bifurcation;
        for(std::vector<std::vector<double> > :: iterator it = bifurcation.begin(); //(std::vector<double> )
                it != bifurcation.end();
                ++it){
            for(std::vector<double>::iterator it_in = it->begin(); it_in != it->end(); ++it_in){
                ar & *it_in;
            }
        }
        //
        //            ar & kinetic_constant;
        for(std::vector<double>::iterator it = kinetic_constant.begin(); it != kinetic_constant.end(); ++it){
            ar & *it;
        }
