...
\\
\hline\\
\key{Output\_precission} & \type{int} & 0 &
$\ldots$
\\
\hline\\
...
\key{Specie\_name} & \type{string} & specie\_i &
name
\\
\hline
\key{dGf} & \type{double} & 0.0 &
...
\key{Reaction\_type} & \type{string} & eq/kin &
type of reaction
\\
\hline
\key{Stechiometry} & \type{int} & 0 &
